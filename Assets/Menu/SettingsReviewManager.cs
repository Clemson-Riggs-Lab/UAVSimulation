...
{
    [SerializeField] public GameObject settingsContainer;
    [SerializeField] public GameObject keyValuePairPrefab;
    private Dictionary<string,KeyValuePairController> _settingsDatabase = new Dictionary<string,KeyValuePairController>();
    
    private void OnValidate()
    {
        MyDebug.AssertComponentReferencedInEditor(settingsContainer,this.gameObject);
        ...
        MessageType valueMessageType = MessageType.None)
    {
        ...
        {
            var keyValuePairGameObject = Instantiate(keyValuePairPrefab, settingsContainer.transform);

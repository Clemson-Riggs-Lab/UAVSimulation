    ...
    public class InputFilesDropDownManager : MonoBehaviour
    {
        ...
        public void PopulateDatabaseAndDropdown(List<FileInfo> filesInfo, bool initialized=true)
        {
            foreach (var fileInfo in filesInfo)
            {
                ...
                {
                    var search = _filesDatabase.Where(i => ((i.Value.Name == fileInfo.Name) && (i.Value.FullName == fileInfo.FullName)));
                    if ( !search.Any()) // no hits on search => similar names with different paths
                    {
            ...
            else RefreshDropDown(selectedIndex: dropdown.options.Count-1);//set selected to last option (most recently added)
        }
